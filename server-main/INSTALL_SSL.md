# 🔒 Установка SSL сертификата (HTTPS)

**Цель:** Установить SSL сертификат для домена `centraldispatch.id` чтобы сайт работал по HTTPS.

---

## ⚡ Быстрая установка (выполнить на сервере)

```bash
# 1. Подключиться к серверу
ssh centraldispatch

# 2. Установить Certbot (если не установлен)
apt update
apt install -y certbot python3-certbot-nginx

# 3. Получить SSL сертификат (автоматически настроит Nginx)
certbot --nginx -d centraldispatch.id \
  --non-interactive \
  --agree-tos \
  --email admin@centraldispatch.id \
  --redirect

# 4. Проверить что SSL работает
curl -I https://centraldispatch.id
```

**Готово!** После этого сайт будет доступен по HTTPS: `https://centraldispatch.id`

---

## 📋 Подробная инструкция

### Шаг 1: Подключиться к серверу

```bash
ssh centraldispatch
# или
ssh root@72.60.31.149
```

### Шаг 2: Установить Certbot

```bash
# Обновить список пакетов
apt update

# Установить Certbot и плагин для Nginx
apt install -y certbot python3-certbot-nginx

# Проверить установку
certbot --version
```

### Шаг 3: Проверить что сайт работает по HTTP

```bash
# Проверить что сайт доступен
curl http://centraldispatch.id

# Должен вернуть HTML или JSON от приложения
```

**⚠️ Важно:** Certbot может получить сертификат только если сайт доступен по HTTP и DNS настроен правильно!

### Шаг 4: Получить SSL сертификат

**Вариант А: Автоматическая установка (рекомендуется)**

```bash
certbot --nginx -d centraldispatch.id \
  --non-interactive \
  --agree-tos \
  --email admin@centraldispatch.id \
  --redirect
```

**Что делает эта команда:**
- ✅ Получает SSL сертификат от Let's Encrypt
- ✅ Автоматически обновляет конфигурацию Nginx
- ✅ Настраивает HTTPS (порт 443)
- ✅ Настраивает редирект с HTTP на HTTPS
- ✅ Настраивает автообновление сертификата

**Вариант Б: Интерактивная установка**

```bash
certbot --nginx -d centraldispatch.id
```

Следуй инструкциям:
- Введи email (для уведомлений о истечении сертификата)
- Согласись с условиями (Y)
- Выбери редирект с HTTP на HTTPS (2 - рекомендуемый вариант)

### Шаг 5: Проверить установку

```bash
# 1. Проверить установленные сертификаты
certbot certificates

# Должен показать:
# Certificate Name: centraldispatch.id
# Domains: centraldispatch.id
# Expiry Date: (дата через 90 дней)
# Certificate Path: /etc/letsencrypt/live/centraldispatch.id/fullchain.pem

# 2. Проверить конфигурацию Nginx
nginx -t

# 3. Проверить HTTPS
curl -I https://centraldispatch.id

# Должен вернуть:
# HTTP/2 200
# (или редирект, если настроен)

# 4. Проверить редирект с HTTP на HTTPS
curl -I http://centraldispatch.id

# Должен вернуть:
# HTTP/1.1 301 Moved Permanently
# Location: https://centraldispatch.id
```

### Шаг 6: Проверить в браузере

Открой в браузере: `https://centraldispatch.id`

**Ожидаемый результат:**
- ✅ Зеленый замочек в адресной строке
- ✅ "Подключение защищено"
- ✅ Сайт открывается по HTTPS

---

## 🔄 Автообновление сертификата

Certbot автоматически настраивает автообновление сертификатов через cron.

**Проверить автообновление:**
```bash
# Тестовый запуск обновления (не обновляет реально)
certbot renew --dry-run

# Должен показать что обновление работает
```

**Сертификаты обновляются автоматически каждые 60 дней** (сертификат действителен 90 дней).

---

## 🐛 Решение проблем

### Проблема 1: Certbot не может получить сертификат

**Ошибка:** "Failed to obtain certificate"

**Решение:**
```bash
# 1. Проверить что сайт доступен по HTTP
curl http://centraldispatch.id

# 2. Проверить DNS
dig centraldispatch.id
# Должен показать IP сервера: 72.60.31.149

# 3. Проверить что порт 80 открыт
netstat -tlnp | grep :80

# 4. Проверить конфигурацию Nginx
nginx -t

# 5. Попробовать снова
certbot --nginx -d centraldispatch.id --non-interactive --agree-tos --email admin@centraldispatch.id
```

### Проблема 2: Ошибка "Too many requests"

**Ошибка:** "Too many requests" от Let's Encrypt

**Причина:** Превышен лимит запросов сертификатов (5 в неделю на домен)

**Решение:**
- Подожди несколько дней
- Или используй существующий сертификат (если есть)

### Проблема 3: Сертификат установлен, но HTTPS не работает

```bash
# 1. Проверить конфигурацию Nginx
nginx -t

# 2. Проверить что Nginx перезагружен
systemctl status nginx

# 3. Перезагрузить Nginx
systemctl reload nginx

# 4. Проверить логи
tail -50 /var/log/nginx/error.log

# 5. Проверить что порт 443 открыт
netstat -tlnp | grep :443
```

### Проблема 4: Certbot находит существующий сертификат

Если Certbot спрашивает:
```
What would you like to do?
1: Attempt to reinstall this existing certificate
2: Renew & replace the certificate
```

**Выбери вариант 1** (рекомендуется):
- Переустановит существующий сертификат
- Не создаст новый (экономит лимиты)
- Быстро настроит Nginx

**Выбери вариант 2** только если:
- Сертификат устарел
- Есть проблемы с текущим сертификатом

---

## 📝 Проверка после установки

```bash
# 1. Проверить сертификаты
certbot certificates

# 2. Проверить конфигурацию Nginx
cat /etc/nginx/sites-available/centraldispatch.id | grep -A 10 "listen 443"

# 3. Проверить HTTPS
curl -I https://centraldispatch.id

# 4. Проверить редирект
curl -I http://centraldispatch.id

# 5. Проверить в браузере
# Открой: https://centraldispatch.id
```

---

## ✅ Чеклист установки SSL

- [ ] Подключен к серверу
- [ ] Certbot установлен
- [ ] Сайт доступен по HTTP
- [ ] DNS настроен правильно
- [ ] SSL сертификат получен
- [ ] Nginx конфигурация обновлена
- [ ] HTTPS работает (`curl -I https://centraldispatch.id`)
- [ ] HTTP редиректит на HTTPS
- [ ] В браузере зеленый замочек
- [ ] Автообновление настроено (`certbot renew --dry-run`)

---

## 🚀 Быстрая команда (все в одной)

Если хочешь установить SSL быстро, выполни:

```bash
ssh centraldispatch "apt update && apt install -y certbot python3-certbot-nginx && certbot --nginx -d centraldispatch.id --non-interactive --agree-tos --email admin@centraldispatch.id --redirect && certbot certificates"
```

---

**После установки SSL сайт будет доступен по HTTPS: `https://centraldispatch.id`** 🔒✨

